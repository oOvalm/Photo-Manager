...
部分地方将lambda表达式改成了方法引用`this::funcion`。
...
- [ ] 奇妙bug：图片缩放，不断放大缩小，图片的最小会一直变小(可能是`Transform`内部实现和想象中的不一样)。**可能的办法**：直接查图片当前大小判断能不能继续缩放
...
2. 幻灯片播放
    - [x] 主窗口幻灯片播放按钮
    - [ ] __(add)播放当前文件夹下的所有图片(含子文件夹内的)__~~tmd怎么感觉要大改~~   [和这个一样？](#jmp2)
    - [x] 双击图片，新窗口显示图片
    - [x] 幻灯片窗口
        - [x] 图片展示
        - [x] 操作功能
